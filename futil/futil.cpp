...
wml::futil::fileExists (const std::string& path)
{
        ...
        if (buf == NULL) {
                DBG ("Memory allocation error in futil::fileExists");
                return false;
        }
...
wml::futil::filesOpen (pid_t pid)
{
        ...
        vector<string> files;
        try {
                futil::readDirectoryTree (files, path);
